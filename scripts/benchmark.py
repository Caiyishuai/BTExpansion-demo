...
if __name__ == '__main__':
    ...
    # print("Planning Time Total:", planning_time_total, "Average:", round(planning_time_total/1000))
    # print("*** Average Number of States:", round(np.mean(total_state_num),2), round(np.std(total_state_num),2))  # 1000次问题的平均状态数
    # print("*** Average Number of Actions:", round(np.mean(total_action_num), 2),  round(np.std(total_state_num), 2))  # 1000次问题的平均状态数
    # print("*** Tree Size: mean=", round(np.mean(total_tree_size),2), "std=", round(np.std(total_tree_size, ddof=1),2))  # 1000次测试树大小

    print("成功次数：", success_count, "失败次数：", failure_count)  # 算法成功和失败次数
    print("总步数：平均值=", np.mean(total_steps_num), "标准差=", np.std(total_steps_num, ddof=1))
    print("总规划时间：", planning_time_total, "平均时间：", round(planning_time_total / 1000))
    print("*** 平均状态数：", round(np.mean(total_state_num), 2), round(np.std(total_state_num), 2))  # 1000次问题的平均状态数
    print("*** 平均动作数：", round(np.mean(total_action_num), 2), round(np.std(total_state_num), 2))  # 1000次问题的平均状态数
    print("*** 树的大小：平均值=", round(np.mean(total_tree_size), 2), "标准差=",
          round(np.std(total_tree_size, ddof=1), 2))  # 1000次测试树大小
